...
<style>
#infoCentro { border: 2px solid #0066cc; border-radius: 8px; padding: 10px 15px; margin: 15px 0; background-color: #f0f8ff; font-size: 14px; }
#infoCentro p { margin: 5px 0; }
#infoCentro span { color: #003366; font-weight: bold; }
...
<div id="infoCentro">
    ...
    <p><strong>Código de centro:</strong> <span><?= htmlspecialchars($cod_centro) ?></span></p>
    <p><strong>Nombre:</strong> <span><?= htmlspecialchars($centro['nombre_centro']) ?></span></p>
    <p><strong>Tipo:</strong> <span><?= htmlspecialchars($centro['tipo_centro']) ?></span></p>
    <p><strong>Localización:</strong> <span><?= htmlspecialchars($centro['loc']) ?></span></p>
</div>
...
async function cargarGrupos() {
    const res = await fetch('../../backend/grupos/get_grupos.php', { method: 'GET', credentials: 'same-origin' });
    const cuerpo = document.getElementById('cuerpoTabla');
    cuerpo.innerHTML = '';

    const data = await res.json();
    if (!data.success) { document.getElementById('mensaje').textContent = data.message; return; }
    if (!Array.isArray(data.grupos) || data.grupos.length === 0) { cuerpo.innerHTML = '<tr><td colspan="7">No hay grupos disponibles</td></tr>'; return; }
    ...
    for (const g of data.grupos) {
        ...
        for (let i = 1; i <= g.listado; i++) {
            const alumno = g.alumnos && g.alumnos[i-1] ? g.alumnos[i-1] : { 
                id_user: idUser, tipo_creado: false, tiene_materias: false,
                fase0_completada: false, faseDUA_completada: false, fecha_ultima_edicion: null
            };
            ...
            const linkFase0 = !alumno.faseDUA_completada ? 'Fase 0' : 'No disponible';
            const linkClassFase0 = !alumno.faseDUA_completada ? 'set-alumno' : 'bloqueado';
    ...
    // Delegación de eventos para .set-alumno
