...
requireAuth();
require_roles(['Director','Tutor']); // Solo Director o Tutor
...
// Validar grupo único para tutores
if (!$error && $rol === 'Tutor') {
    $stmt2 = $pdo->prepare("SELECT COUNT(*) FROM ti_gr1 WHERE id_user = ?");
    $stmt2->execute([$user_id]);
    if ($stmt2->fetchColumn() > 0) {
        $error = "Los tutores solo pueden crear un grupo. Ya existe un grupo creado.";
    }
}

// Contar cuántos grupos ya tiene creados el usuario
$stmt = $pdo->prepare("SELECT COUNT(*) FROM ti_gr1 WHERE id_user = ?");
$stmt->execute([$user_id]);
$numGrupos = (int) $stmt->fetchColumn();

// Director con ya 5 grupos
